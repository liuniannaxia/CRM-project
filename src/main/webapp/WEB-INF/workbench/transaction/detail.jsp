...
<body>
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 20px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;" id="stage"><b></b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
...
<script>
	//异步查询交易历史，交易备注等信息
	$.post("/crm/workbench/transaction/queryDetail",{'id':'${id}'},function (data) {
		$('#stage').html(data.stage)
		$('#possibility').html(data.possibility)

		var transactionHistorys = data.transactionHistories;
		for (var i = 0 ; i<transactionHistorys.length;i++){
			var transaction = transactionHistorys[i];
		}
		$('#stageHistory').append("<tr>\n" +
				"\t\t\t\t\t\t\t<td>"+transaction.stage+"</td>\n" +
				"\t\t\t\t\t\t\t<td>"+transactionHistorys.money+"</td>\n" +
	...
	//异步查询阶段图标
	//传id 号是为了获取当前阶段的图标和可能性，所有阶段的图标和可能性从session里面获取
	$.post("/crm/workbench/transaction/stageList",{'id':'${id}'},function (data) {
		refresh(data);
...
//点击阶段图标，改变当前阶段的状态
	function changeStage(index,stage,possibility) {
		//要把点击那个图标的索引传到后台
		//索引位置从后台设置，返回给前台
		$.post("/crm/workbench/transaction/stageList",{'id':'${id}','index':index},function (data) {
			refresh(data);
			$('#stage').html(stage)
	...
	function refresh(data) {
		...
		for (var i = 0;i<stages.length;i++){
			...
			if (stage.type == "绿圈"){	//下面onclick函数要用\"包裹起来，转义
				count +="<span onclick=\"changeStage('"+stage.index+"',+'"+stage.stage+"','"+stage.possibility+"')\" class=\"glyphicon glyphicon-ok-circle mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stage.stage+":"+stage.possibility+" style=\"color: #90F790;\"></span>"
			}else if (stage.type == "锚点"){
				count +="<span onclick=\"changeStage('"+stage.index+"')\" class=\"glyphicon glyphicon-map-marker mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stage.stage+":"+stage.possibility+" style=\"color: #90F790;\"></span>"
			}else if (stage.type == "黑圈"){
				count +="<span onclick=\"changeStage('"+stage.index+"','"+stage.stage+"','"+stage.possibility+"')\" class=\"glyphicon glyphicon-ok-circle mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stage.stage+":"+stage.possibility+" style=\"color: #000;\"></span>"
			}else if (stage.type == "黑x"){
				count +="<span onclick=\"changeStage('"+stage.index+"','"+stage.stage+"','"+stage.possibility+"')\" class=\"glyphicon glyphicon-remove mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stage.stage+":"+stage.possibility+" style=\"color: #000;\"></span>"
			}else{
				count +="<span onclick=\"changeStage('"+stage.index+"','"+stage.stage+"','"+stage.possibility+"')\" class=\"glyphicon glyphicon-remove mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content="+stage.stage+":"+stage.possibility+" style=\"color: red;\"></span>"
			}
			count +="-----------"
		}
		count +="<span class=\"closingDate\">"+new Date().toLocaleDateString()+"</span>"
		$('#stageDiv').html(count);
		...
		var transactionHistory = data['transactionHistory']
		$('#stageHistory').append("<tr>\n" +

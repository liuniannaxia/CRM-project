		...
		<div class="modal-dialog" role="document" style="width: 90%;">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal" id="createClueForm" role="form">
							...
							<div class="col-sm-10" style="width: 300px;">
								<select class="form-control" id="create-call"name="appellation">
									<c:forEach items="${map['appellation']}" var="appellation">
										<option ${appellation.value}>${appellation.text}</option>
									</c:forEach>
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 300px;">
								<select class="form-control"name="status" id="create-status">
									<c:forEach items="${map['clueState']}" var="clueState">
										<option ${clueState.value}>${clueState.text}</option>
									</c:forEach>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" onclick="createClue()" data-dismiss="modal">保存</button>
				</div>
			</div>
		</div>
	</div>
				...
				<table class="table table-hover">
					...
					<tbody>
						<tr>
							<td><input type="checkbox" /></td>
							<%--														这里需要传参数id，--%>
							<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href='/crm/toView/workbench/clue/detail?id=cb05424e43104d018499f2a20e55d36e';">张三教授</a></td>
							<td>动力节点</td>
							<td>010-84846003</td>
							<td>12345678901</td>
							<td>广告</td>
...
<script>
	//点击保存按钮，创建线索
	function createClue() {
		//表单序列化，需要每个用户提交的都有name属性，并且这个name是和数据库中字段对应的
		var form = $('#createClueForm').serialize();
		$.post("/crm/workbench/clue/creatClue",form,function (data) {
				if (data.isOk){
					alert("添加线索成功");
				} else {
					alert("添加线索失败")
	...
	function queryClue() {
		...
		$.post("/crm/workbench/clue/queryClue",form,function (data) {
			if (data.isOk){
				//吧查询出来的数据拼接页面
			}

		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" id="searchInput" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"onclick="relateActivity()" data-dismiss="modal">关联</button>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	function refreshActivity(){
		//发送异步请求，查询线索关联的市场活动		  这里的EL表达式需要加引号，从本页面获取的值
		$.get("/crm/workbench/clue/queryClueActivity",{'id':'${id}'},function (data) {
            $('#tbody').html("");
	...
	//给关联市场活动输入框绑定回车事件，查询满足条件的市场活动
	$('#searchInput').keypress(function (event) {
        if(event.keyCode==13){
            //发送异步请求查询市场活动                          这里this指的是 输入框   市场活动的id号和名字模糊查询
            $.post("/crm/workbench/queryAllActivitys",{'name':$.trim($(this).val()),'id':'${id}'},function (data) {
                //清空上一次查询的数据
                $('#relateActivity').html("");
                for(var i = 0;i<data.length;i++){
                    var activity = data[i];
                    $('#relateActivity').append("<tr>\n" +
    ...
    //关联市场活动
    function relateActivity() {
	    //先获取勾中的个数,然后放到数组里，才能以  ， 分隔开
        var ids = [];
        $('.son:checked').each(function () {
            ids.push($(this).val());
        })
        //发送异步请求，把数据插入中间表
        $.post("/crm/workbench/clue/relateActivity",{'ids':ids.join(),        //勾中的市场活动id
                    'id':'${id}'},          //线索的id
            function (data) {
            if(data.isOk){
                alert("关联成功")
...
//bug
    //解除绑定       删除中间表的记录（根据  市场id和线索id删除）
    function unbind(activityId) {

        //发送异步请求，把中间表数据删除
        $.post("/crm/workbench/clue/deleteActivity",{'id':'${id}',        //勾中的市场活动id
                                                     'activityId':activityId},          //线索的id
            function (data) {

                var result = confirm("确定解除绑定吗");
                    //开始删除
                if(result =true&&data.isOk){
                    alert("解绑成功")
                    //刷新页面
                    refreshActivity();
                }
